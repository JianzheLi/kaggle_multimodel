...
FP_MULTIOME_TEST_INPUTS = os.path.join(DATA_DIR,"train_multi_targets.h5")
...
df_cite_target_data = pd.read_hdf(FP_MULTIOME_TEST_INPUTS)
...
def input_preprocessing():
    ...
    removed_data=preprocessing_remove(combined_data)
    combined_data_normalized = normalize_by_nonzero_median(removed_data)
    
    #combined_data = pd.concat([df_multi_train_x_normalized, df_multi_test_x_normalized], axis=0)

# 进行tSVD降维
    combined_data_reduced_1 = perform_tsvd(combined_data_normalized)


# 将非零值转换为 1
    
    removed_data_binary = removed_data.applymap(lambda x: 1 if x != 0 else 0)

    combined_data_reduced_2=perform_tsvd(removed_data_binary)
...
def target_preprocessing():
    row_medians = np.median(df_cite_target_data[df_cite_target_data != 0], axis=1)
    row_medians[row_medians == 0] = 1  # 防止除以零
    median_divided_data = df_cite_target_data.div(row_medians, axis=0)
...
# 生成列名
    ...
    reduced_data_df = pd.DataFrame(reduced_data, index=df_cite_target_data.index, columns=column_names)
